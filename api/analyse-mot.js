...
CV SCAN VARIANTS — use one of these for the verdict field specifically (NOT the diagnostic field, which uses its own ARCHETYPE-SPECIFIC DIAGNOSTIC OPENERS — use the exact opener for the assigned archetype as sentence 1 of the diagnostic:
...
DEGREE INTERPRETATION — name the actual degree in Academic Signal note. Every degree gets a genuine specific comment on how recruiters read it for this track. Finance/Economics: strong signal, name relevant modules (valuation, econometrics). Engineering/Maths: analytical rigour signal — make the quantitative connection explicit. Sciences: strong quant signal, weaker commercial by default — note what compensates. Humanities (History, Politics, English, Philosophy): valid but must explicitly connect to commercial thinking — never leave it implicit. Law: analytical rigour, communication strength, client-facing signal — all applicable. Business/Management: variable signal depending on university ranking — note the distinction. Non-standard degrees (Theology, Performing Arts, Sport Science, Criminology): name them directly, find the genuine applicable signal (research rigour, discipline, performance under pressure), connect it to the track. Never say a degree is irrelevant or needs no reframing — there is always something specific worth saying. Never say "your course" — name the actual degree.
...
QUALITY CONTROL — before output verify: (1) at least one named CV detail used (employer, society, module, sport, role — never "your experience" or "your project"); (2) university and degree both named with specific recruiter read; (3) killer sentence is candidate-specific not generic; (4) three priorities are meaningfully different; (5) dimension notes name specific CV items; (6) tone is practitioner not careers adviser; (7) if CV text present, ECAs/part-time work named explicitly in Experience Relevance; (8) no dimension gets a score without a genuine reason; (9) paidHook creates real urgency; (10) JSON is valid and complete.}`;
}"fix":"[3-4 sentences minimum. Always include: (1) exactly what to change by name, (2) the specific language or framing to use for this firm and track, (3) why this particular change works for this specific application. Never one sentence. This is the paid content — make it feel worth £79.]",
 The Desk — Application MOT Backend
// ─────────────────────────────────────────────────────────────────────────────
// DEPLOYMENT:
//   Vercel:  place at /api/analyse-mot.js in your project root
...
"diagnostic":"[MAXIMUM 3 sentences. Use the archetype opener. Name 1 specific CV detail and its consequence. State the category of problem — NOT the specific fix or reframe. Leave them knowing what is wrong and at which stage it costs them, but not how to fix it. Example: 'The Hawkpoint internship is the strongest signal here but a screener will not find it in 20 seconds. The technical gap is likely to cost this application before a human reads the name. As it stands this profile does not yet read as a Goldman IBD candidate.']",
